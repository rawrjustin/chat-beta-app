...
export function SuggestedPromptsBar({
  ...
  const containerClasses = useMemo(() => {
    ...
    const baseClasses =
      'transition-all duration-300 ease-out transform max-w-4xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 pt-3 sm:pt-4 pb-3 sm:pb-4';
        ...
        {prompts.map((prompt, index) => {
          ...
          const accentColor =
            prompt.type === 'roleplay'
              ? 'bg-gradient-to-r from-purple-50 via-pink-50 to-purple-100 border-purple-200 hover:border-purple-300'
              : 'bg-gradient-to-r from-sky-50 via-blue-50 to-indigo-100 border-blue-200 hover:border-blue-300';
          ...
          return (
            <button
              ...
              disabled={disabled}
              className={`w-full sm:flex-1 px-4 sm:px-5 py-2.5 sm:py-3 text-left text-sm sm:text-base font-medium text-gray-800 border rounded-full shadow-sm transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 hover:shadow-md disabled:opacity-60 disabled:cursor-not-allowed ${accentColor}`}
